...
Comparative runs use the frozen `terse-v1` same-goal instruction as the primary
baseline. Select `LIVE_EVAL_BASELINE=previous-skill` with
`LIVE_EVAL_PREVIOUS_SKILL_DIR`, or `LIVE_EVAL_BASELINE=no-instruction`, only
when that regression or causal question is the one being tested. Supply
`LIVE_EVAL_MODEL`, `LIVE_EVAL_MODEL_CONFIG`, and
`LIVE_EVAL_TASK_DISTRIBUTION` for a claim-ready exact identity. See the
[comparative evaluation workspace](evals/README.md) for bounded ablations,
artifact layout, anomaly semantics, and the iteration loop.
...
The live runner executes case trials concurrently while counterbalancing candidate/baseline generation order across repeated trials and keeping dependencies inside each trial ordered. It logs case, trial, and phase start/completion, elapsed time, and aggregate progress to stderr and persists the same timestamped stream in ignored `evals/results/live-progress.log`, without echoing prompts or fixture contents. Every candidate and baseline uses its own disposable matched task-workspace copy with skill runtime surfaces, eval definitions, results, and unrelated fixtures removed. Selected case fixtures are supplied to both arms through their prompts and materialized at matching workspace paths when execution is useful; only skill-backed arms receive a selected or snapshotted skill bundle. Each Codex arm and judge uses a distinct private temporary home containing only the authentication file, and judges also run from separate empty temporary directories. The runner rejects unsafe fixture paths and symbolic links and applies per-command timeouts with process-group cleanup. On normal exit, `SIGHUP`, `SIGINT`, or `SIGTERM`, it removes active process trees, temporary workspaces, and credential copies. A hard kill or host failure can still leave temporary credential residue that must be removed manually. Each run writes a versioned, bounded, redacted workspace under `evals/results/runs/<run-id>/`; `live-latest.json` is only the rolling pointer and summary. Keep live evaluation out of the fast CI gate because it depends on local agent installation, credentials, model availability, and cost. Use `npm run eval:sample` to generate the same artifact shape without a paid model call.
