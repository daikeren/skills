...
Comparative run 會使用固定的同目標 `terse-v1` instruction 作為主要 baseline。只有在要回答 regression 或不同 causal question 時，才使用 `LIVE_EVAL_BASELINE=previous-skill` 搭配 `LIVE_EVAL_PREVIOUS_SKILL_DIR`，或使用 `LIVE_EVAL_BASELINE=no-instruction`。要讓 run 具備可支援 claim 的精確 identity，請提供 `LIVE_EVAL_MODEL`、`LIVE_EVAL_MODEL_CONFIG` 與 `LIVE_EVAL_TASK_DISTRIBUTION`。Bounded ablation、artifact layout、anomaly semantics 與 iteration loop 詳見 [comparative evaluation workspace](evals/README.md)。
