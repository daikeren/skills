...
7. Run a bounded grilling pass when the direction is fuzzy: challenge overloaded terms, invent concrete edge-case scenarios, cross-check stated behavior against code or docs when available, and expose hidden tradeoffs. Track unresolved decisions by dependency without exhaustively enumerating every possible branch.
8. Ask only the current blocking frontier: decisions whose prerequisites are settled and whose answers can change scope, design, rollout, or risk. Defer questions that depend on another unsettled decision, ask one at a time when possible, and include a recommended answer when it materially lowers decision cost. If safe assumptions unblock low-risk work, state them and continue; if a user-owned decision is missing for high-risk work, stop and ask.
