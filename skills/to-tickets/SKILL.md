...
9. Give every ticket blocking edges. A ticket with no blockers is part of the frontier and can start immediately.
10. Use expand-contract sequencing for wide refactors, schema migrations, API transitions, or compatibility changes. Expand the new path beside the old one, migrate callers in safe batches, then contract the old path after all blockers are clear.
11. Make each ticket independently reviewable and mergeable. If a ticket cannot be released independently, say what gate, flag, integration branch, or final verify ticket makes that safe.
12. Human owns release, priority, policy, and irreversible decisions; agent owns dependencies, facts, and risks. If the user is absent, proceed only with explicitly recorded low-risk assumptions and leave high-risk decisions as blockers.
13. Include tests and verification per ticket. Add cleanup follow-ups only after compatibility and rollout are safe.
14. For cross-session handoff, prefer the repo's existing ticket convention. If none exists, propose a path and minimal frontmatter instead of forcing file creation.
15. Before treating the list as final, check granularity: too coarse, too fine, wrong blockers, or tickets that should be merged or split. If the user is not available, include those as review questions.
16. Order tickets by blockers and release sequence, not by architecture layers.
