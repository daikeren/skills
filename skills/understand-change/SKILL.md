...
4. Select from the following elements to design the smallest useful learning path; do not turn them into mandatory sections for a simple explanation:
   ...
   In a medium cross-layer chat explanation, prefer one end-to-end trace plus a compact contract map. Do not restate the same behavior separately as background, walkthrough, checkpoints, debugging guidance, and recap; add a second representation only when it teaches a distinct relationship.
